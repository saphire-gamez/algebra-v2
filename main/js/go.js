  ...
  window.navigator.serviceWorker.register('./sw.js', {
    ...
    if(url != ""){
      if(localStorage.getItem("ab_cloak") == "true") {
        var tab = window.open('about:blank', '_blank');
        tab.document.documentElement.innerHTML = '<!DOCTYPE html><html><head><title>' + /*(localStorage.getItem("tabCloakTitle") ? localStorage.getItem("tabCloakTitle") : "Utopia")*/'Algebra-lessons.com' + '</title><link rel="icon" type="image/png" href="' + /*(localStorage.getItem("tabCloakIcon") ? localStorage.getItem("tabCloakIcon") : window.location.origin + "/favicon.ico")*/window.location.origin + "/favicon.ico" + '"><style>body {margin:0;overflow:hidden}</style></head><body><iframe width="100%" height="100%" src="' + window.location.origin + __uv$config.prefix + encodeXor(this.url) + '" frameborder="0"></iframe></body></html>';
        tab.document.close();
